...
`scrape_dawn.py` automatically builds the Oxylabs auth user as `customer-<username>` (standard Oxylabs format).  
If your account already provides a fully formatted username, disable the prefix:

```bash
python scrape_dawn.py --proxy-auth-prefix ""
```
